...
BEGIN
    ...
    WHERE ta.TaskID = NEW.TaskID
    ORDER BY ta.AssignedAt DESC, ta.TaskAssignmentID DESC
    LIMIT 1;
...
Planned Approach:Use a trigger on Robot insertion or update that checks for existing active role records for the same robot. If another active role exists, the operation is rejected or the previous role is automatically deactivated before allowing the new assignment.
 */
...
Planned Approach: Implement a insert  trigger on the TaskExecution table that checks for the existence of a corresponding TaskAssignment record for the same robot and task. If no valid assignment is found, the operation is rejected. A helper function may be used to verify assignment validity.
 */
...
Planned Approach:Implement an insert trigger on the Alert table. When a new alert is inserted with high severity, trigger automatically inserts a corresponding record into the SupportRequest table.
 */
